...
"""Packed E2M1/UE8M0 MQA logits with BF16 compute.

Ported from CherryLemon/vllm, commit a9e3d217cce0, file
vllm/model_executor/kernels/attention/dsa/sm90_fp4_indexer.py.
The segregated page format is 64 payload bytes and 4 scale bytes per
128-dimensional token. Q contains low/high nibbles along the reduction
axis; Q scales contain four raw UE8M0 bytes per int32.

The BF16 rounding boundaries, candidate masks, group-6 admission and
workspace visibility semantics follow the source. The decode uses integer
operations without NVIDIA inline PTX. Only H100 is validated so far.
This API returns dense logits; it does not claim bounded workspace at
arbitrary context length. It is distinct from FP8-Q/FP4-K DSA operators.
"""
...
def _e2m1_decode(code):
    ...
    # Canonicalize both zero nibbles without an NVIDIA inline-PTX add.
...
@triton.jit
def _mxfp4_paged_index_logits_kernel(
    q_ptr,
    qs_ptr,
    w_ptr,
    cache_ptr,
...
@triton.jit
def _mxfp4_grouped_paged_index_logits_kernel(
    q_ptr,
    qs_ptr,
    w_ptr,
    cache_ptr,
    ...
    if not USE_CANDIDATES:  # noqa: SIM102
        if lb * BLOCK_L >= tl.minimum(max_n_vis, width):
            for j in tl.range(0, GROUP, loop_unroll_factor=1):
                b = first + j
                if b < n_rows:
                    _mxfp4_group_invalid(out_ptr, b, offs_l, width, stride_out, BLOCK_L)
            return
    ...
    if share:
        k_low, k_high, logical, col_valid = _mxfp4_group_load_keys(
            cache_ptr,
            block_table_ptr,
            candidate_blocks_ptr,
            first,
...
@triton.jit
def _mxfp4_workspace_index_logits_kernel(
    q_ptr,
    qs_ptr,
    w_ptr,
    k_values_ptr,
...
def mxfp4_paged_index_logits(
    ...
    *,
    ...
    if use_grouped:
        ...
        logger.debug(
            "MXFP4 indexer: grouped (group-6) kernel launched "
        ...
        _mxfp4_grouped_paged_index_logits_kernel[grid](
            q_values,
            q_scale_bytes,
            weights,
            cache,
    ...
    _mxfp4_paged_index_logits_kernel[grid](
        q_values,
        q_scale_bytes,
        weights,
        cache,
...
def mxfp4_workspace_index_logits(
    q_values: torch.Tensor,
    q_scale: torch.Tensor,
    weights: torch.Tensor,
    ...
    *,
    ...
    grid = (rows, triton.cdiv(width, _BLOCK_L))
    _mxfp4_workspace_index_logits_kernel[grid](
        q_values,
        q_scale_bytes,
        weights,
        k_values,
